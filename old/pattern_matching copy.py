...
def parse(text):
    parsed_text = parse_text(text.lower())    
    print(parsed_text)
...
def parse_transformation(transformation):
    ...
    for p in transformation.parameters:
        results.append(p.replace('_', ' '))

    # then parse description
    
    results += parse(re.sub('[\(\)\[\]]', ' ', transformation.description))
    for p in transformation.parameters:
        results.append(parse(re.sub('[\(\)\[\]]', ' ', transformation.parameters[p])))

    return results

def parse_entry(entry): # meaning needs pair of N, ADJ (ADV ADJ)
